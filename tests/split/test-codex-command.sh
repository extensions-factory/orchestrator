...
grep -Fq 'Both require `context.base_sha`; `security_review` additionally requires `context.security_focus`.' "$SKILL"
grep -Fq 'A missing `base_sha` is a malformed request which must not be degraded or substituted.' "$SKILL"
grep -Fq 'Review commands stay foreground and are never backgrounded' "$SKILL"
