...
check_dispatch "$RC" D14 "role: tech_lead" "task_type: code_review_quality" "task review" "re-review"
check_dispatch "$RC" D15 "role: security_engineer" "task_type: security_review" "only when" "security-sensitive"
check_dispatch "$SDD" D16 "role: software_engineer" "plan task's task_type" "Critical/Important findings" "D14"
check_dispatch "$RC" D17 "role: tech_lead" "task_type: code_review_quality" "whole-branch review" "D18"
check_dispatch "$SDD" D18 "role: software_engineer" "task_type: implementation_coding" "one fix wave" "D17"

check_dispatch "$FB" D19 "role: devops_engineer" "task_type: release_deployment" "selected finish path" "chosen option"
