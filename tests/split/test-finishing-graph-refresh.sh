...
check 'use `.understand-anything/knowledge-graph.json` only when `.understand-anything/` exists' 'legacy graph path is conditional'
check 'compare `project.gitCommitHash` with `git log -1 --format=%H -- .`' 'post-land freshness uses scoped HEAD'
check 'If the graph is absent or malformed (including a missing or null `project.gitCommitHash`), skip the gate and continue without error.' 'absent or malformed graph degrades silently'
