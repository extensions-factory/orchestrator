...
const SERVER_PATH = path.join(__dirname, '../../skills/visual-companion/scripts/server.cjs');
  ...
  try {
    ...
    await test('helper.js defines required APIs', () => {
      const helperContent = fs.readFileSync(
        path.join(__dirname, '../../skills/visual-companion/scripts/helper.js'), 'utf-8'
      );
    ...
    await test('frame template has required structure', () => {
      const template = fs.readFileSync(
        path.join(__dirname, '../../skills/visual-companion/templates/frame-template.html'), 'utf-8'
      );
