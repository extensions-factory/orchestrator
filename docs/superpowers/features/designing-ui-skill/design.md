...
- Does not auto-chain from `brainstorming`; the user or orchestrator invokes it directly.
...
- GIVEN the user has reviewed and accepted the spec WHEN the skill ends THEN the agent invokes `superpowers-orchestrator:writing-plans` and does not invoke any other skill.
...
- SC-5: The skill's terminal state is always `superpowers-orchestrator:writing-plans` — no other skill is invoked on successful completion.
