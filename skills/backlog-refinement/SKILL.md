...
**Dispatch:** `D23` sends the current roadmap and only work the human explicitly adds through `superpowers-orchestrator:dispatch-agent` with `role: product_owner` and `task_type: backlog_refinement_prioritization` to propose ordering and grooming; the Scrum Master presents the proposal, applies it to `roadmap.json`/`ROADMAP.html` only after the human approves, and when there is no worker provider selected or ready, `superpowers-orchestrator:dispatch-agent` degrades to the always-available claude subagent.
