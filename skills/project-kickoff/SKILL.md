...
- Write `CONSTITUTION.md` from [constitution-template.md](constitution-template.md) — the single canonical source of truth for the Phase 2 standards answers.
- Write **thin per-tool instruction files** for each selected AI tool (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, …) from [tool-instruction-template.md](tool-instruction-template.md), each a pointer to `CONSTITUTION.md` plus any genuinely tool-specific config.
