...
- `--background` runs rescue across turns so a long task is not killed by the foreground wall-clock cap; fetch its result with `/codex:status` then `/codex:result` (see §3). Review commands keep `--wait` (foreground) — they are fast and read-only.
...
For rescue, `--background` + `/codex:status` + `/codex:result` are the routed path.
Never use `/codex:cancel`, resume, `--profile`, or transfer; never background a review
command. Never substitute one command family for another.
